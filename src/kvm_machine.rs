...
fn get_disks(xml: &str, device_name: &str) -> Result<DiskImage> {
    ...
    for device_node in disk_nodes {


    }

    let disk = device_nodes
...
fn get_disks(xml: &str, device_name: &str) -> Result<DiskImage> {
    ...
    let device_nodes =  doc.descendants()
    ...
    let disk = device_nodes.descendants()
        .filter(|x| x.has_tag_name("disk"))
        .filter_first(|x| x.children().filter(|y| {
            y.has_tag_name("target") && y.attribute("dev") == Some(device_name)
        }).has_any())
